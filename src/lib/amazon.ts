...
export async function getAmazonProducts(marketplace:Marketplace):Promise<ProductResponse>{
 ...
 try{
  ...
  return{...base,products:[],count:0,live:false,fetchedAt:null,message:error instanceof Error&&error.message==='credentials_missing'?'Live Amazon products are not configured yet.':marketplace==='de'?'Amazon live products are temporarily unavailable. Please try again shortly; no cached prices are being shown.':'Amazon has not yet enabled this marketplace for the connected Creators API account. No prices or fallback products are being shown.'};
 }
}
